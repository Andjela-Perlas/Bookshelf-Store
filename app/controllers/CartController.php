...
class CartController extends Controller {
	...
	public function getNewAmount() {
		...
		try{
			$userID = $_SESSION['user']->userID;
			$cartModel = new Cart($this->conn);
			$data = $cartModel->totalPrice($userID);
		} catch(PDOException $e){
			echo $e->getMessage();
		}
	...
	public function cart() {
		...
		$total = $cartModel->totalPrice($userID);
		$cartNumber = $cartModel->getNumberOfBooks($userID);

		$this->view("cart", ["menu" => $menu, "networks" => $networks, "products" => $cart, "total" => $total, "cartNumber" => $cartNumber]);
	}
}
